...
class Store
  ...
  observe: (key, fun) ->
    ...
    console.log("Sending command: #{cmd.show()}")
    @runtime.send(cmd.show())
  ...
  handleNotify: (cmd) ->
    ...
    console.log(">>> Store Handling Notify for #{scmd}")
...
# No so elegant, but there were issues with the map of lambdas.
  handleCommand: (cmd) ->
    scmd = JSON.stringify(cmd)
    console.log("handle-command: #{scmd}")
    switch cmd.cid
      when 'NOK'
        this.handleNOK(cmd)
      when 'OK'
